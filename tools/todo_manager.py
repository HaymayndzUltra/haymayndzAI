...
def _save(data: List[Dict[str, Any]]) -> None:
	# Atomic, locked write to prevent concurrent truncation/corruption
	with with_json_lock(str(DATA_FILE), exclusive=True):
		atomic_write_json(str(DATA_FILE), data)
...
def exec_substep(task_id: str, sub_index: str, run: bool = False) -> None:
	...
	print("🚀 Executing...")
	from repo.exec_logging import run_logged
	for cmd in selected:
		try:
