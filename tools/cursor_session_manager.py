...
class CursorSessionManager:
	...
	DEFAULT_STATE_FILE = os.path.join(os.getcwd(), "memory-bank", "cursor_state.json")
	...
	def __init__(self, state_file: str | None = None, autosave_interval: int = 30):
		...
		# Ensure target directory exists for state file
		try:
	...
	def _save_state_to_disk(self) -> None:
		with self._lock:
			...
			try:
				...
				pass
			# Atomic write via temp + replace as before
			...
			try:
				...
				try:
					...
					pass
				# Mirror to root path if it exists (compatibility only)
				root_cursor_state = os.path.join(os.getcwd(), "cursor_state.json")
				if os.path.exists(root_cursor_state):
					with with_json_lock(root_cursor_state, exclusive=True):
						atomic_write_json(root_cursor_state, self._state)
			except OSError:
