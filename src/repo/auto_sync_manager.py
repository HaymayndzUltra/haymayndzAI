...
class AutoSyncManager:
	...
	def _update_cursor_state(self, active_task: Optional[Dict[str, Any]]):
		...
		try:
			...
			# Atomic, locked write
			with with_json_lock(self.state_files['cursor_state'], exclusive=True):
				atomic_write_json(self.state_files['cursor_state'], cursor_state)
			# Also update root level if it exists (compatibility mirror)
			root_cursor_state = 'cursor_state.json'
			if os.path.exists(root_cursor_state):
				with with_json_lock(root_cursor_state, exclusive=True):
					atomic_write_json(root_cursor_state, cursor_state)
		except Exception as e:
	...
	def _update_task_state(self, active_task: Optional[Dict[str, Any]]):
		...
		try:
			...
			if active_task:
				...
				task_state = {}
			with with_json_lock(self.state_files['task_state'], exclusive=True):
				atomic_write_json(self.state_files['task_state'], task_state)
		except Exception as e:
	...
	def _update_task_interruption_state(self, active_task: Optional[Dict[str, Any]]):
		...
		try:
			...
			with with_json_lock(self.state_files['task_interruption'], exclusive=True):
				atomic_write_json(self.state_files['task_interruption'], interruption_state)
		except Exception as e:
...
"""
			...
			# Single writer policy: call bridge to write session file
	...
	def cleanup_duplicate_tasks(self) -> Dict[str, Any]:
		...
		try:
			...
			# Atomic, locked write of active tasks
			with with_json_lock(self.state_files['active_tasks'], exclusive=True):
				atomic_write_json(self.state_files['active_tasks'], cleaned_tasks)
